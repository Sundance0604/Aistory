...
def capture_conversation(page, url, cid):
    """Fetch the full conversation JSON directly using the logged-in session."""

    # Capture the same auth headers used by ChatGPT's own frontend.
    # In Business workspaces this also includes chatgpt-account-id.
    auth = capture_auth(page)

    api_url = f"{BASE_URL}/backend-api/conversation/{cid}"

    res = fetch_with_session(page, api_url, auth)

    log(f"FETCHED: {res['status']} {api_url}")

    if res["status"] != 200:
        return None, auth, res["status"]

    try:
        data = json.loads(res["body"])
    except Exception as e:
        raise RuntimeError(
            f"Conversation response is not valid JSON: {e}"
        )
    ...
    if (
        ...
        raise RuntimeError(
            f"Invalid conversation response: "
            f"status={res['status']}, "
            f"keys={list(data.keys()) if isinstance(data, dict) else type(data)}"
        )

    return data, auth, 200
    ...
    ap.add_argument("--fix-files", action="store_true",
                    help="don't re-export conversations; instead revisit every "
                        "already-exported chat and fetch any missing attachments, "
                         "and generated (code-tool) files")
    ap.add_argument("--include-library", action="store_true",
                    help="explicitly opt into sweeping the account File Library "
                         "(disabled by default)")
