...
export function History({ onConversation }: { onConversation: (id: string) => void }) {
  const [sort, setSort] = useState('total_visible_tokens')
  const [rows, setRows] = useState<Conversation[]>([])
  const [records, setRecords] = useState<any>(null)
  useEffect(() => { api<Conversation[]>(`/api/rankings/conversations?sort=${sort}&limit=100`).then(setRows); api('/api/records').then(setRecords) }, [sort])
  return (
