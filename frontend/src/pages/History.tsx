...
const allSorts = [['total_messages', '交互事件'], ['outbound_messages', '主动事件'], ['inbound_messages', '响应事件'], ['active_days', '活跃日'], ['calendar_span_days', '生命周期']]

export function History({ provider, onConversation }: { provider: string; onConversation: (id: string) => void }) {
  const [sort, setSort] = useState(provider === 'chatgpt' || provider === 'gemini' ? 'total_visible_tokens' : 'total_messages')
  const [rows, setRows] = useState<Conversation[]>([])
  const [records, setRecords] = useState<any>(null)
  useEffect(() => { setSort(provider === 'chatgpt' || provider === 'gemini' ? 'total_visible_tokens' : 'total_messages') }, [provider])
  ...
  const messageMode = isWechat || isAll
  const sorts = isWechat ? wechatSorts : isAll ? allSorts : aiSorts
  return (
    <div className="page-stack">
      ...
      <section className="panel">
        ...
        <div className="ranking-table">
          <div className="ranking-head"><span>#</span><span>对话</span><span>{isWechat ? '我发送' : isAll ? '主动' : '提示'}</span><span>{isWechat ? '收到' : isAll ? '响应' : '输入'}</span><span>{messageMode ? '活跃日' : '输出'}</span><span>{messageMode ? '总事件' : '总计'}</span></div>
