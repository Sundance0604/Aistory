...
export function ActivityPage({ initial, provider, onConversation }: { initial: Day[]; provider: string; onConversation: (id: string) => void }) {
  ...
  useEffect(() => { setMetric(isWechat || isAll ? 'total_messages' : 'total_visible_tokens') }, [isWechat, isAll])
  ...
  return <div className="page-stack">
    ...
    <section className="panel">
      <div className="panel-heading wrap"><div><span className="eyebrow">当前视图总计</span><h2>{compact(total)}</h2></div><div className="controls">
        ...
        <select value={metric} onChange={event => setMetric(event.target.value as Metric)}>{isWechat || isAll ? <><option value="outbound_messages">{isWechat ? '我发送' : '主动事件'}</option><option value="inbound_messages">{isWechat ? '收到' : '响应事件'}</option><option value="total_messages">总事件</option></> : <><option value="prompts">提示数</option><option value="prompt_visible_tokens">输入可见 Token</option><option value="response_visible_tokens">输出可见 Token</option><option value="total_visible_tokens">总可见 Token</option></>}<option value="active_conversations">活跃对话</option></select>
      </div></div>
      <MiniBars data={data} metric={metric} onSelect={select} />
      <div className="activity-table-scroll"><div className="activity-table"><span>周期</span><span>{isWechat ? '我发送' : isAll ? '主动' : '提示'}</span><span>{isWechat ? '收到' : isAll ? '响应' : '输入'}</span><span>{isWechat || isAll ? '总计' : '输出'}</span>{[...data].reverse().map(row => <button className={`table-row ${selected===row.date?'selected':''}`} onClick={() => select(row.date)} key={row.date}><b>{row.date}</b><span>{compact(isWechat || isAll ? row.outbound_messages : row.prompts)}</span><span>{compact(isWechat || isAll ? row.inbound_messages : row.prompt_visible_tokens)}</span><span>{compact(isWechat || isAll ? row.total_messages : row.response_visible_tokens)}</span></button>)}</div></div>
    </section>
    {selected && granularity === 'daily' && <section className="panel"><div className="panel-heading"><div><span className="eyebrow">{selected}</span><h2>当日对话</h2></div><span>{details.length} 个对话</span></div><div className="day-conversations">{details.map(row => <button onClick={() => onConversation(row.id)} key={row.id} style={{'--topic-color': row.provider === 'wechat' ? (row.conversation_type === 'group' ? '#8E7FA6' : '#6D9AA3') : row.topic_color || '#888'} as React.CSSProperties}><span className="topic-dot"/><div><b>{row.title}</b><small>{row.account_display_name || row.account_name} · {row.provider === 'wechat' ? (row.conversation_type === 'group' ? '群聊' : '私聊') : `${row.provider.toUpperCase()} · ${row.dominant_topic || '未分类'}`}</small></div><strong>{row.provider === 'wechat' ? `${compact(row.outbound_messages)} 我发送 · ${compact(row.total_messages)} 消息` : `${compact(row.prompts)} 提示 · ${compact(row.total_visible_tokens)} Token`}</strong></button>)}</div></section>}
  </div>
}
