...
export function Settings() {
  ...
  return (
    <div className="page-stack narrow">
      ...
      <section className="panel form-panel"><h2>全局使用时长模型</h2><p className="notice">时长由所有平台的用户提示合并成全局时间线后推断；不会把对话或平台切换当作硬边界。</p><label>Session 尾部余量（分钟）<input type="number" min="0" value={values.usage_time.tail_allowance_minutes} onChange={(e) => setValues({ ...values, usage_time: { ...values.usage_time, tail_allowance_minutes: Number(e.target.value) } })}/></label><label>最少 gap 样本<input type="number" min="2" value={values.usage_time.min_model_samples} onChange={(e) => setValues({ ...values, usage_time: { ...values.usage_time, min_model_samples: Number(e.target.value) } })}/></label><p className="privacy-note">修改后保存会重新生成模型。尾部余量是可见的估计假设，不是观测事实。</p></section>
