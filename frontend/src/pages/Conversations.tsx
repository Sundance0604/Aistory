...
export function Conversations({ provider, requestedId, onClose }: { provider: string; requestedId?: string; onClose?: () => void }) {
  ...
  useEffect(() => { const timer = setTimeout(() => api<Conversation[]>(`/api/conversations?sort=${sort}&limit=500&search=${encodeURIComponent(search)}&account_id=${encodeURIComponent(accountId)}&provider=${provider}`).then(setRows), 150); return () => clearTimeout(timer) }, [search, sort, accountId, provider])
  ...
  return (
    <div className="page-stack">
      ...
      {detail && <div className="drawer-backdrop" onClick={() => { setDetail(null); onClose?.() }}><aside className="drawer" onClick={(event) => event.stopPropagation()}><button className="icon-button close" onClick={() => { setDetail(null); onClose?.() }}><X /></button><span className="eyebrow">{detail.account_name} · {detail.provider.toUpperCase()} · {dateLabel(detail.created_at)}</span><h2>{detail.title}</h2><div className="drawer-metrics"><span><b>{compact(detail.prompts)}</b> 提示</span><span><b>{compact(detail.total_visible_tokens)}</b> Token</span></div><div className="topic-chips">{detail.topics?.map(topic => <span style={{'--topic-color': topic.color} as React.CSSProperties} key={topic.id}>{topic.name}</span>)}</div><div className="messages">{detail.messages.filter((message) => message.role === 'user' || message.visible_text).map((message) => <article className={message.role} key={message.id}><header><b>{message.role === 'user' ? '你' : detail.provider === 'gemini' ? 'Gemini' : 'ChatGPT'}</b><span>{compact(message.visible_tokens)} tokens</span></header><p>{message.visible_text || (message.has_attachment ? '［非文本输入/附件］' : '［无可见文本］')}</p></article>)}</div></aside></div>}
    </div>
  )
}
